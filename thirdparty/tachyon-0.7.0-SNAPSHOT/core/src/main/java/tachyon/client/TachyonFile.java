...
public class TachyonFile implements Comparable<TachyonFile> {
  ...
  private TachyonByteBuffer readLocalByteBuffer(int blockIndex, long offset, long len)
    ...
    LOG.debug("readLocalByteBuffer : local file name = '" + localFileName + "'");
    ...
    LOG.debug("readLocalByteBuffer : going to return null buffer for local buffer = '"
            + localFileName + "'");
    mTachyonFS.unlockBlock(blockId, blockLockId);
    return null;
  }
  ...
  boolean recache(int blockIndex) throws IOException {
    ...
    LOG.debug("TachyonFile '" + path + "' is requested for recaching");
    ...
    try {
      ...
      LOG.debug("TachyonFile '" + path + "' : execption occur while recache : '"
              + e.getMessage() + "'");
      if (bos != null) {
        bos.cancel();
      }
