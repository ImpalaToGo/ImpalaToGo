...
# Starts up an statestore with the specified command line
# arguments. An optional -build_type parameter can be passed to determine the build
# type to use for the statestored instance.
...
GDB_PREFIX=""
IMPALAD_BINARY=statestore/statestored
...
do
  case "$ARG" in
    ...
    -in-process)
      ...
      ;;
    -gdb)
      echo "Starting statestore under gdb..."
      GDB_PREFIX="gdb --args"
      ;;
    -jvm_debug_port=*)
      JVM_DEBUG_PORT="${ARG#*=}"
