...
void JsonDelimitedTextParser::addColumnInternal(int len, char** data, int* num_fields,
	...
	if(m_schema_defined && *data == NULL && len == 0 && key.empty()){
		...
		if(m_number_of_materialized_fields == m_schema.size())
			return;

		for(boost::unordered_map<std::string, SchemaMapping>::const_iterator it = m_schema.begin(); it != m_schema.end(); ++it){
			if(!get_bit(m_tuple, (*it).second.column_idx)){
				...
				field_locations[*num_fields].idx = (*it).second.llvm_tuple_idx;

				// set the bit busy in bitmap
				set_bit(m_tuple, (*it).second.column_idx);
