/*
 * @file delimited-text-json-parser-test.cc
 *
 * @brief Contains test cases for JSON parser functionality
 *
 * @date   Apr 15, 2015
 * @author elenav
 */
...
#include "exec/delimited-text-json-parser-test-fixtures.hpp"

using namespace std;

namespace impala {

void JsonParserTest::validate(const std::string& data,
...
TEST_F(JsonParserTest, Batch_0_no_delimiters) {
	char TUPLE_DELIM = '|';

	reset(-1, TUPLE_DELIM);
	validate("{\"no_delims\":100}", -1, TUPLE_DELIM, 0, 0, 0);
}
 ...
 */
TEST_F(JsonParserTest, Plain_JSON_simple_fields_truncated_batches_1) {
	char TUPLE_DELIM = '|';

	reset(5, TUPLE_DELIM);

	// say 1 incompleted row is expected
	validate("|{\"field1\":120, \"field2\":\"text\",\"fi", 1, TUPLE_DELIM, 0, 2, 1);
...
*/
TEST_F(JsonParserTest, Plain_JSON_simple_fields_truncated_batches_2) {
	char TUPLE_DELIM = '|';

    reset(2, TUPLE_DELIM);

    validate("|{\"field1\":120, \"field2\":\"text\",\"", 1, TUPLE_DELIM, 0, 2, 1);
...
*/
TEST_F(JsonParserTest, Plain_JSON_simple_fields_truncated_batches_3) {
	char TUPLE_DELIM = '|';

    reset(2, TUPLE_DELIM);
	validate("|{\"field1\":\"te\", \"field2\":\"text\",", 1, TUPLE_DELIM, 0, 2, 1);
 ...
 */
TEST_F(JsonParserTest, Plain_JSON_array_fields_truncated_batches) {
	char TUPLE_DELIM = '|';
    ...
    */
	validate("|{\"simple\":\"text\",\"arr\":[12,", 1, TUPLE_DELIM, 0, 2, 2);
