...
/*
 * @file delimited-text-parser-raw.inline.h
 *
 * @brief Contains inlined raw parser implementations of DelimitedTextParser interface
 *
 * @date   Apr 9, 2015
 * @author elenav
 */

#ifndef SRC_EXEC_DELIMITED_TEXT_PARSER_RAW_INLINE_H_
#define SRC_EXEC_DELIMITED_TEXT_PARSER_RAW_INLINE_H_
...
inline void RawDelimitedTextParser::ParseSse(int max_tuples,
  ...
  while (LIKELY(*remaining_len >= SSEUtil::CHARS_PER_128_BIT_REGISTER)) {
    ...
    char* last_char = *byte_buffer_ptr + (SSEUtil::CHARS_PER_128_BIT_REGISTER - 1);
    bool last_char_is_unescaped_delim = delim_mask >> (SSEUtil::CHARS_PER_128_BIT_REGISTER - 1);
...
inline void RawDelimitedTextParser::parseSingleTupleInternal(int64_t remaining_len, char* buffer,
  ...
  if (LIKELY(CpuInfo::IsSupported(CpuInfo::SSE4_2))) {
    while (LIKELY(remaining_len >= SSEUtil::CHARS_PER_128_BIT_REGISTER)) {
      ...
      while (delim_mask != 0) {
        ...
        delim_mask &= ~(SSEUtil::SSE_BITMASK[n]);
        if(process_escapes_flag)
        	AddColumn<true>(buffer + n - next_column_start,
        			&next_column_start, num_fields, field_locations);
        else
        	AddColumn<false>(buffer + n - next_column_start,
        	            &next_column_start, num_fields, field_locations);
  ...
  while (remaining_len > 0) {
    ...
    if (!last_char_is_escape_ &&
        ...
        if(process_escapes_flag)
        	AddColumn<true>(buffer - next_column_start,
        			&next_column_start, num_fields, field_locations);
        else
        	AddColumn<false>(buffer - next_column_start,
        	            &next_column_start, num_fields, field_locations);
    }
  ...
  if(process_escapes_flag){
	  FillColumns<true>(buffer - next_column_start,
			  &next_column_start, num_fields, field_locations);
  }
  else{
	  FillColumns<false>(buffer - next_column_start,
			  &next_column_start, num_fields, field_locations);
  }
}
...
#endif /* SRC_EXEC_DELIMITED_TEXT_PARSER_RAW_INLINE_H_ */
